...
unsigned int hash_function(char* palavra, unsigned int len){
  return BKDRHash(palavra, len)%HASH_SIZE;
}
...
bool conferePalavra(const char *palavra) {
    ...
    char *p = (char *)palavra;
    for ( ; *p; ++p) *p = tolower(*p);
    int bucket = hash_function((char *)palavra, tam_palavra);
    cursor = hashtable[bucket];
    while (cursor != NULL){
        if (strcmp(palavra, cursor->palavra) == 0) return true;
        cursor = cursor->prox;
    }
    return false;
} /* fim-conferePalavra */
...
bool carregaDicionario(const char *dicionario) {
    ...
    while (!feof(fp)){
        ...
        new_node->prox = NULL;
        strcpy(new_node->palavra, buffer);
        int bucket = hash_function(new_node->palavra, strlen(new_node->palavra));
        printf("%s\n",buffer );
        if (hashtable[bucket] == NULL){
            hashtable[bucket] = new_node;
        }else{
            new_node->prox = hashtable[bucket];
            hashtable[bucket] = new_node;
        }
        cont++;
    }
